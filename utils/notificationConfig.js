...
class NotificationConfig {
  ...
  async get(key, envKey = null) {
    try {
      // Check cache first
      if (this.cache[key] && Date.now() - this.lastFetch[key] < this.cacheExpiry) {
  ...
  async getAll() {
    try {
      const settings = await SettingsService.getByCategory('notification');
      const config = {};

      settings.forEach(setting => {
        config[setting.key] = setting.getParsedValue();
      });
      ...
      return {
        ...
        smtp_secure: config.smtp_secure !== undefined ? config.smtp_secure : (process.env.SMTP_SECURE === 'true'),
        ...
        whatsapp_enabled: config.whatsapp_enabled !== undefined ? config.whatsapp_enabled : (process.env.WHATSAPP_ENABLED === 'true'),
        admin_phone: config.admin_phone || process.env.ADMIN_PHONE,
        admin_url: config.admin_url || process.env.ADMIN_URL || 'http://localhost:3000'
      };
    } catch (error) {
